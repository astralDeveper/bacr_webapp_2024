        ...
        {CompanyProfileData.map((item, ind) => (
          <div className={``}>
            ...
            {item.bullets.map((items) => (
              <ul id={`profile-${ind}`} className={`text-[13px] pl-10 lg:text-[14px] text-[#000] font-light mx-auto md:mb-4 mb-2 profile  ${visibleCards.includes(`profile-${ind}`) ? "visible" : ""}  `}>
                <li className=" list-disc">{items}</li>
              </ul>
            ))}
          </div>
        ))}
        <div className="bg-backgroundColor1 flex items-center py-2  mx-auto md:pr-6 pr-3  rounded-lg "  >
          <Button onclick={onButtonClick} title={"Download Company Profile"} btnStyle="text-backgroundColor2 text2 uppercase font-semibold " onClick={onButtonClick} />
